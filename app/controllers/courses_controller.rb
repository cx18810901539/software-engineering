class CoursesController < ApplicationController
  ...
  def list
    ...
    if params.has_key?(:course_time)
      ...
      if ctime == "不限" and ctype != "不限" and cname == ""
        @courses = Course.where("open = ? and course_type like ? and id not in (?)", true, ctype, excluded_courses_id).paginate(page: params[:page], per_page: 4)
      elsif ctime == "不限" and ctype != "不限" and cname != ""
        @courses = Course.where("open = ? and course_type like ? and name like ? and id not in (?)", true, ctype, "%#{cname}%", excluded_courses_id).paginate(page: params[:page], per_page: 4)
      elsif ctime == "不限" and ctype == "不限" and cname != ""
  ...
  def select
    ...
    current_user.courses.each do |c|
      ...
      if c.name[0..3] == @course.name[0..3]
        flash={:warning => "您过去已选择了课程：#{c.name}"}
        redirect_to list_courses_path, flash: flash
        return
      end
    end
